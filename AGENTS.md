...
- 旅行视频帧或旅行照片转视觉；
- 用户点名 `gc-minimal-zine-poster` 并要求横版 16:9；
...
1. 默认 `16:9` 横版，不得回退为上游 `3:5` 竖版；
2. 必须保留原图关键场景证据，例如地形、岸线、建筑体块、人物姿态、地平线或标志性物件；
3. 每张只选择一个主要视觉语法；批量时不得连续机械重复同一语法和同一贴纸位置；
4. 强调色优先从原图提取，不得固定套蓝色；
5. 已知地点时，在留白区加入英文地名；
6. 地点未知时不得编造；不得虚构坐标、天气、日期、档案号或其他精确信息；
7. 抖音 UI、账号名、点赞评论、播放器控件、手机黑边和上下对比界面都不是源画面内容，必须排除；
8. 结果必须是重新组织视觉语法后的 Zine 画面，不得只是原照片叠纸纹理；
9. 批量输出保持纸张和字体系统一致，同时让构图、意象与强调色随源画面变化；
10. 上游许可与本地改动说明见 `skills/gc-minimal-zine-travel-16x9/SOURCE.md` 和 `LICENSE`。
