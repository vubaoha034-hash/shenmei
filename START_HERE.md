...
## F. 旅行视频 / 照片 16:9 极简 Zine

适用：

- 旅行视频帧转视觉；
- 风景、人物旅行照转横版 Zine；
- 用户点名 `gc-minimal-zine-poster` 并要求 16:9；
...
- 默认输出 `16:9` 横版，不得回退成 3:5 竖版；
- 保留原画面的地形、岸线、建筑、人姿态或标志性物件等关键证据；
- 每张只选择一个主视觉语法，批量时轮换，避免连续重复；
- 主强调色优先从原图提取；
- 已知地点时在留白中加入英文地名；
- 不得虚构坐标、天气、日期、档案编号或其他精确信息；
- 排除抖音 UI、播放器控件、手机框和上下对比界面；
